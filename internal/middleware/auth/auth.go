...
func (auth *Auth) GenOkMsg(c *gin.Context, desc string) any {
...
func (auth *Auth) GenErrMsg(c *gin.Context, desc string, err error) any {
	auth.Logger.FailWithField(c, core.Unauthorized, desc, err)
	return gin.H{
		"status": gin.H{
			"code": core.OK,
			"desc": core.FormatError(core.Unauthorized, desc, err).Error(),
		},
	}
}
