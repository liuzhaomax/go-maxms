...
func Cors() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if ext.In(whiteList, c.Request.Header.Get("Origin")) {
			c.Header("Access-Control-Allow-Origin", c.Request.Header.Get("Origin"))
			c.Header(
				"Access-Control-Allow-Headers",
				"Content-Type, AccessToken, X-CSRF-Token, Authorization, Token, Set-Cookie, X-Requested-With, Access-Control-Allow-Origin, Content-Security-Policy, Request_id, App_id, User_id",
			)
