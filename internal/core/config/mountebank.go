...
func (m *mountebankConfig) CreateImposter(stubDir string) {
	// 读取imposter请求体
	v := viper.New()
	...
	mbURL := fmt.Sprintf(
		"%s://%s:%s/imposters",
		cfg.Lib.Mountebank.Protocol,
		cfg.Lib.Mountebank.Mb.Endpoint.Host,
		cfg.Lib.Mountebank.Mb.Endpoint.Port,
	)
...
func (m *mountebankConfig) DeleteImposter(stubDir string) {
	// 读取imposter请求体
	v := viper.New()
	v.SetConfigFile(stubDir)
	...
	// 发送请求
	req, err := http.NewRequest(http.MethodDelete, mbURL, nil)
	if err != nil {
